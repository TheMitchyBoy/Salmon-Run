...
const BUNDLED_PREVIEW = path.join(__dirname, '..', 'marker-reference.jpg');
...
function seedManifest() {
  ...
  const previewPath = getPreviewPath(id);
  if (fs.existsSync(BUNDLED_PREVIEW) && !fs.existsSync(previewPath)) {
    fs.copyFileSync(BUNDLED_PREVIEW, previewPath);
  }
...
function restoreBundledDefault() {
  ...
  const previewPath = getPreviewPath(id);
  if (fs.existsSync(BUNDLED_PREVIEW)) {
    fs.copyFileSync(BUNDLED_PREVIEW, previewPath);
  }
